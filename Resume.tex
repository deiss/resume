\documentclass[a4paper, 12pt]{article}

% english
\usepackage[english]{babel}
\usepackage{lmodern}

% packages
\usepackage{fullpage}       % really narrow margins
\usepackage{graphicx}       % \includegraphics
\usepackage{amsmath}        % \text
\usepackage{multirow}       % \multirow
\usepackage{enumitem}       % \setitemsize
\usepackage{appendix}       % appendix env
\usepackage{pgf, tikz}      % figures
\usepackage{hyperref}       % url

% margin
\usepackage{geometry}
\geometry{
  a4paper,
  left=10mm,
  right=10mm,
  top=10mm,
  bottom=10mm
}

% margin
\setlength\parindent{0pt}                         % no indentation
\setitemize{itemsep=-0.1cm}                       % no space between itms
\newcommand{\marginline}{-0.3cm}
\newcommand{\margincontent}{-0.6cm}
\newcommand{\margincontentnotextbefore}{-0.3cm}
\newcommand{\marginbeforesection}{0.35cm}
\newcommand{\marginmisc}{0.3cm}
\newcommand{\marginbeforemisc}{-0.75cm}
\newcommand{\linewidthperso}{0.02cm}

% styles
\newcommand{\styletitle}[1]{\textbf{#1}}
\newcommand{\styledesc}[1]{\textit{#1}}
\newcommand{\styleloc}[1]{\textbf{#1}}
\newcommand{\styledates}[1]{\textit{#1}}
\newcommand{\stylesummary}[1]{
  \vspace{\marginbeforesection}
  \begin{normalsize}\textit{#1}\end{normalsize}
  \vspace{\marginline}\\

}
\newcommand{\stylesection}[1]{
  \vspace{\marginbeforesection}
  \begin{normalsize}\textit{#1}\end{normalsize}
  \vspace{\marginline}\\
  \noindent\makebox[\linewidth]{\rule{\textwidth}{\linewidthperso}}

}

% document
\begin{document}
\pagenumbering{gobble}
\begin{footnotesize}

\begin{center}
  \begin{small}\textbf{OLIVIER DEISS}\end{small} \textbullet \:\begin{small}\textbf{US CITIZEN}\end{small}\\
  olivier.deiss@gmail.com \textbullet \:\url{http://github.com/deiss}\\
\end{center}

\stylesection{Summary}

\vspace{\margincontentnotextbefore}
\begin{itemize}
  \item 8 years of experience leading teams on Spanner \& Persistent Disk, some of the largest sytems at Google.
  \item MSc. in Computer Science from Georgia Tech, MEng. in Electrical and Computer Engineering from Sup\'elec.
\end{itemize}

\stylesection{Professional Experience}

\styletitle{GOOGLE} \hfill \styleloc{Kirkland, WA, USA}\\
\styledesc{Staff Software Engineer -- Technical Infrastructure, Spanner} \hfill \styledates{Dec. 2025 -- Present}\\
\vspace{\margincontent}
\begin{itemize}
  \item Technical Lead of Spanner Lookups, overseeing Reads (main API with 4B QPS) and CRUD SQL queries.
  \item Leading research \& implementation of latency and cost optimizations.
\end{itemize}
\styledesc{Senior Software Engineer -- Google Cloud Platform, Persistent Disk} \hfill \styledates{Aug. 2018 -- Dec. 2025}\\
\vspace{\margincontent}
\begin{itemize}
  \item Technical Lead of the disk optimization backend, byte efficiency efforts, and on-disk format.
  \item Main inventor of Patent No. US 12,417,016: compactions for instant snapshots that preserve block deduplication.
  \item Enabled write-through caching to reduce latency of read-modify-write requests for 16 kiB disks used by Cloud SQL.
\end{itemize}
\styledesc{Software Engineering Intern -- Assistant, Android TV} \hfill \styledates{Jan. 2017 -- Apr. 2017}\\
\vspace{\margincontent}
\begin{itemize}
  \item Designed and implemented a messaging infrastructure between Assistant Server and Android TV (\textit{C++}, \textit{Java}).
  \item Integrated this infrastructure as a preferred alternative to Cast, to be used for major media queries on Android TV.
  \item Enabled launching native apps, including YouTube and Netflix, from a query to Google Home.
\end{itemize}

\styletitle{THOMSON REUTERS} \hfill \styleloc{London, UK}\\
\styledesc{Software Developer Intern, Technology, Eikon Financial Analysis} \hfill \styledates{Jul. 2016 -- Aug. 2016}\\
\vspace{\margincontent}
\begin{itemize}
  \item Implemented a module to replace error messages for unauthorized content with a list of available products (\textit{Polymer}).
  \item Built demo apps to introduce the feature to customers (\textit{Node.js}, \textit{Neo4j}, \textit{Python} and \textit{Mocha}/\textit{Chai} for unit tests).
  \item My work has been shown to James Smith, CEO of Thomson Reuters, and won an internal innovation contest.
\end{itemize}

\styletitle{INSTRUCTOR LIVE} \hfill \styleloc{London, UK}\\
\styledesc{Full-Stack Developer Intern} \hfill \styledates{Jun. 2015 -- Aug. 2015}\\
\vspace{\margincontent}
\begin{itemize}
  \item Developed a new video player for the Android and Samsung Smart TV apps, supporting a new content provider.
  \item Implemented a new user type on the webserver that integrates the new business model on the back-end logic.
  \item Designed a workout planner and a calendar module for the front-end, encouraging subscriptions (\textit{Angular}).
\end{itemize}

\stylesection{Education}

\styletitle{GEORGIA INSTITUTE OF TECHNOLOGY} \hfill \styleloc{Atlanta, GA, USA}\\
\styledesc{Master of Science in Computer Science} \hfill \styledates{Aug. 2016 -- May. 2018}\\
\vspace{\margincontent}
\begin{itemize}
  \item Graduate Research Assistantship and Master's Thesis with Dr. Jimeng Sun: \hfill GPA: 4.0\\
  Labeling techniques and interpretability of neural networks applied to the classification of seizures using EEGs.
  \item \textit{Fall 2016}: Analysis of rosbags (\textit{ROS}) with \textit{Hadoop} and \textit{C++} jobs, with Dr. C\'edric Pradalier.
\end{itemize}

\styletitle{SUP\'ELEC} \hfill \styleloc{Metz, FRA}\\
\styledesc{Master of Engineering in Electrical and Computer Engineering} \hfill \styledates{Sep. 2014 -- Jul. 2018}\\
\vspace{\margincontent}
\begin{itemize}
  \item Bachelor of Science earned in 2016. \hfill GPA: 3.9
  \item Master of Engineering earned with graduation from Georgia Tech in 2018.
\end{itemize}

\styletitle{LYC\'EE PIERRE CORNEILLE} \hfill \styleloc{Rouen, FRA}\\
\styledesc{Preparatory Classes -- Passed entry exam to Grandes Ecoles} \hfill \styledates{Sep. 2012 -- Jun. 2014}\\
\vspace{\margincontent}
\begin{itemize}
  \item Advanced maths, physics and chemistry. \hfill Rank: 1/45
\end{itemize}

\stylesection{Projects}

\styletitle{DIGITAL ART GALLERY PLATFORM FOR BTL MEDIA} \hfill \styleloc{Seattle, WA, USA}\\
\styledesc{Supporting my wife's artistic endeavours} \hfill \styledates{Mar. 2026 -- current}\\
\vspace{\margincontent}
\begin{itemize}
    \item Gallery \& exhibition management software, with an AI-native codebase (\textit{Angular SPA}, \textit{Node.js} server).
\end{itemize}

\styletitle{BRAIN-COMPUTER INTERFACE} \hfill \styleloc{Metz, FRA}\\
\styledesc{Research at Sup\'elec} \hfill \styledates{Nov. 2015 -- Jun. 2016}\\
\vspace{\margincontent}
\begin{itemize}
  \item Led a research group developing real-time classification of EEG signals (ML, DSP, \textit{C++}).
  \item Achieved 70\% accuracy for simple hand gestures and applied it to the remote control of a robot (\textit{ROS}).
\end{itemize}

\styletitle{MISCELLANEOUS}\\
\vspace{\margincontent}
\begin{itemize}
  \item \textit{DigitScanner:} implemented a neural network from scratch, achieving 98.24\% accuracy on MNIST.
  \item \textit{FFTOcean:} implemented the FFT algorithm for the real-time simulation of ocean water (\textit{C++}, \textit{OpenGL}).
\end{itemize}

\stylesection{Skills \& Interests}
~\\
\vspace{\marginbeforemisc}

\noindent
\textbf{Languages:} \textit{C++}, \textit{Python}, \textit{Java}, \textit{Javascript}\\
\textbf{Interests}: Paragliding, SCUBA Diving, Cats\\

\end{footnotesize}
\end{document}
